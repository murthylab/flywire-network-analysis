...
def switch_and_hold_GE(edges, niters=1000, verbose=False):
    """configuration model while hoding bidegree
...
def switch_and_hold_R3(edges, niters=1000, verbose=False):
    """Iterative implementation of the configuration model, holding bidegees
    ...
    def is_switch_allowed(edges, edge_pair):
        u, v = edge_pair
        # no self-edges created
        if (u[0] == v[1]) or (v[0] == u[1]):
            return 0
        # check if switch is available (uni & bidirectional switches possible)
        x, y = (u[0], v[1]), (v[0], u[1])
        xr, yr = (v[1], u[0]), (u[1], v[0])
        if (x in edges) or (y in edges) or (xr in edges) or (yr in edges):
            return 0
        # check if bidirectional
        ur, vr = (u[1], u[0]), (v[1], v[0])
        if (ur in edges) and (vr in edges):
            return 2
        # check if unidirectional
        elif (ur not in edges) and (vr not in edges):
            return 1
        # one edge is unidirectional, the other bidirectional
        return 0

    def make_unidirectional_switch(edges, edge_pair):
        u, v = edge_pair
        x, y = (u[0], v[1]), (v[0], u[1])
        edges.remove(u)
        edges.remove(v)
    ...
    def make_bidirectional_switch(edges, edge_pair):
        u, v = edge_pair
        ur, vr = (u[1], u[0]), (v[1], v[0])
        make_unidirectional_switch(edges, (u,v))
        make_unidirectional_switch(edges, (ur,vr))
    
    start = time()
    for i in range(niters):
        ep = get_random_edge_pair(edges)
        switch_decision = is_switch_allowed(edges, ep)
        if switch_decision == 1:
            make_unidirectional_switch(edges, ep)
            uni += 1
        elif switch_decision == 2:
            make_bidirectional_switch(edges, ep)
            bi += 1
        else:
            holds += 1
    end = time()
    if verbose:
        print('holds: {}, {:0.2f}'.format(holds, holds / niters))
        print('uni switches: {}, {:0.2f}'.format(uni, uni / niters))
        print('bi switches: {}, {:0.2f}'.format(bi, bi / niters))
        print('time: {:0.2f} s'.format(end-start))
    return edges
...
def continuous_sample_three_neuron_motifs_R3(V, E, samples, niters, thread, i):
    worker_init_fn(i)
    df = pd.DataFrame()
    for s in range(samples):
        E = switch_and_hold_R3(E, niters=niters)
        tdf, _ = count_three_neuron_motifs(V, E, motifs)
        df = pd.concat((df, pd.DataFrame([tdf])), ignore_index=True)
    return df
